...
from common.agent_registry import (
    get_by_type,
    get_by_subagent_name,
    get_by_skill_dir,
    format_agent_name,
    detect_agent_type,
)
...
class AgentTracker:
    ...
    def log_delegation(
        ...
        logger.info("🔄 " + "=" * 76)
        logger.info(f"🔄 [AgentTracker] 📤 从: {format_agent_name(from_agent)}")
        logger.info(f"🔄 [AgentTracker] 📥 到: {format_agent_name(to_agent)}")
    ...
    def log_agent_response(
        ...
        # 输出响应日志
        icon = get_by_type(agent_type).icon
        logger.info("")
    ...
    def detect_and_log_tool_calls(self, messages: List[BaseMessage]):
        ...
        for msg in messages:
            if hasattr(msg, 'tool_calls') and msg.tool_calls:
                for tc in msg.tool_calls:
                    ...
                    all_tool_names.append(f"{name}({file_path})" if file_path else name)
        logger.info(f"[AgentTracker] 消息中的 tool_calls: {all_tool_names}")
        ...
        for i, msg in enumerate(messages):
            ...
            if hasattr(msg, 'tool_calls') and msg.tool_calls:
                for tool_call in msg.tool_calls:
                    ...
                    if tool_name in ("task", "Task"):
                        ...
                        agent_info = get_by_subagent_name(subagent_type)

                        self.log_delegation(
                            from_agent=caller,
                            to_agent=agent_info.display_name,
            ...
            if hasattr(msg, 'type') and hasattr(msg, 'content'):
                agent_type = detect_agent_type(msg.content)
                agent_name = get_by_type(agent_type).display_name
